  ...
  return (
    <div
      ...
      style={{
        ...
        background: "linear-gradient(160deg, oklch(0.14 0.06 255), oklch(0.09 0.04 265))",
        boxShadow: "0 0 14px oklch(0.5 0.12 240 / 0.25), 0 8px 24px rgba(0,0,0,0.45)",
      }}
    >
      <div
        className="pointer-events-none absolute inset-x-0 top-0 h-1/3"
        style={{ background: "linear-gradient(to bottom, oklch(0.6 0.1 240 / 0.08), transparent)" }}
      />
      <div
      ...
      />
      {(["left-[4px] top-[4px]", "right-[4px] top-[4px]", "left-[4px] bottom-[4px]", "right-[4px] bottom-[4px]"] as const).map(
        (pos) => (
          <span
            key={pos}
...
function WheelSpinHint() {
  return (
    <div className="pointer-events-none relative mt-1 flex flex-col items-center gap-1.5 pb-1">
      <motion.div
        className="h-px w-[min(10rem,50vw)] overflow-hidden rounded-full"
        style={{
          background:
            "linear-gradient(to right, transparent, oklch(1 0 0 / 0.2), transparent)",
        }}
        animate={{ scaleX: [0.45, 1, 0.45], opacity: [0.35, 0.7, 0.35] }}
      ...
      <p className="text-center text-[9px] font-medium uppercase tracking-[0.22em] text-white/30">
        Swipe to shuffle · Tap to choose
      </p>
    </div>
  );
}
...
export function TarotSpread({ onAllSelected, onReveal }: TarotSpreadProps) {
  ...
  // Auto-scroll to CTA after all 3 cards land
  useEffect(() => {
    if (!allDone) return;
    const t = setTimeout(
  ...
  const handleSelect = useCallback(
    (card: TarotCard, sourceRect: SourceRect | null) => {
      ...
      if (sourceRect) {
        ...
        setFlights((prev) => [...prev, { id: flyId, card, from: sourceRect, to: destRect }]);
      }
  ...
  return (
    <div className="relative flex w-full flex-col items-center gap-0">
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 0 60px oklch(0.4 0.1 255 / 0.3), 0 24px 48px rgba(0,0,0,0.5)",
              }}
              onClick={(e) => e.stopPropagation()}
            >
              {/* logo */}
              <div className="shrink-0 opacity-80" style={{ width: 96, height: 96 }}>
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button
                  ...
                  style={{
                    background: "linear-gradient(135deg, oklch(0.55 0.15 240), oklch(0.42 0.14 260))",
                    color: "oklch(0.92 0.04 220)",
                    boxShadow: "0 0 18px oklch(0.5 0.15 240 / 0.4), 0 4px 12px rgba(0,0,0,0.3)",
                  }}
                >
      ...
      <div
        ref={wheelWrapRef}
        className="relative w-full overflow-hidden rounded-2xl border border-white/6 bg-black/15 shadow-[0_0_0_1px_oklch(1_0_0/0.03)_inset] backdrop-blur-md"
      >
      ...
      */}
      <div
        ...
        style={{ opacity: spreadVisible ? 1 : 0, pointerEvents: spreadVisible ? "auto" : "none" }}
      >
        <div className="flex items-start justify-center gap-4 sm:gap-6">
          {Array.from({ length: 3 }).map((_, i) => {
            const landed = selected[i] !== undefined;
            const inFlight = !landed && i === selected.length && flights.length > 0;
            return (
              <div
                key={i}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="relative overflow-hidden rounded-full px-8 py-3 text-sm font-medium tracking-wide transition-all active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-70"
        style={{
